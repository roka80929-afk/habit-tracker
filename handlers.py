...
TIME_12H_RE = re.compile(r"^(\d{1,2})[.:](\d{2})$")
...
def ampm_keyboard(prefix: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="🌞 AM (утро/день)", callback_data=f"{prefix}:AM"),
                InlineKeyboardButton(text="🌙 PM (вечер/ночь)", callback_data=f"{prefix}:PM"),
            ]
        ]
    )
...
def parse_12h_time(text: str) -> tuple[int, int] | None:
    """Парсит время вида '9.30' или '09:30' в 12-часовом формате. Возвращает (час 1-12, минута 0-59) или None."""
    match = TIME_12H_RE.match(text.strip())
    if not match:
        return None
...
def convert_12_to_24(hour: int, minute: int, period: str) -> str:
    """period: 'AM' или 'PM'. Возвращает строку 'ЧЧ:ММ' в 24-часовом формате."""
    if period == "AM":
...
async def add_habit_time_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if value == "custom":
        ...
        await callback.message.edit_text("Напиши время в 12-часовом формате, например 9.30")
...
async def add_habit_custom_time(message: Message, state: FSMContext) -> None:
    parsed = parse_12h_time(message.text)
    if parsed is None:
        await message.answer("Не похоже на время. Формат Ч.ММ, например 9.30 (часы от 1 до 12). Попробуй ещё раз:")
        return
    hour, minute = parsed
    await state.update_data(pending_hour=hour, pending_minute=minute)
    await state.set_state(AddHabit.waiting_ampm)
    await message.answer(f"{hour}.{minute:02d} — это утро/день или вечер/ночь?", reply_markup=ampm_keyboard("addampm"))


@router.callback_query(StateFilter(AddHabit.waiting_ampm), F.data.startswith("addampm:"))
async def add_habit_ampm_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    period = callback.data.split(":", 1)[1]
    data = await state.get_data()
    name = data.get("habit_name", "привычка")
    hour = data.get("pending_hour")
    minute = data.get("pending_minute")
    time_str = convert_12_to_24(hour, minute, period)
    await db.add_habit(callback.from_user.id, name, reminder_time=time_str)
    await state.clear()
    await callback.message.edit_text(f"Добавил привычку «{name}» ✅\nНапоминание в {time_str}.")
    await callback.answer()
...
async def change_time_time_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if value == "custom":
        ...
        await callback.message.edit_text("Напиши новое время в 12-часовом формате, например 9.30")
        await callback.answer()
        return
...
async def change_time_custom(message: Message, state: FSMContext) -> None:
    ...
    hour, minute = parsed
    await state.update_data(pending_hour=hour, pending_minute=minute)
    await state.set_state(ChangeTime.waiting_ampm)
    await message.answer(f"{hour}.{minute:02d} — это утро/день или вечер/ночь?", reply_markup=ampm_keyboard("chtimeampm"))


@router.callback_query(StateFilter(ChangeTime.waiting_ampm), F.data.startswith("chtimeampm:"))
async def change_time_ampm_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    period = callback.data.split(":", 1)[1]
    data = await state.get_data()
    habit_id = data.get("habit_id")
    hour = data.get("pending_hour")
    ...
    await state.clear()
    await callback.message.edit_text(f"Готово! Новое время напоминания: {time_str}.")
    await callback.answer()
